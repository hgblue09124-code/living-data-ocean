...
class Administrator:
    ...
    def create_human(self, position: Optional[tuple] = None) -> AdministratorCommand:
        """Tạo lệnh khởi tạo con người mới vào thế giới."""
        cmd = AdministratorCommand(
            command_type="CREATE_HUMAN",
            payload={"position": position or (5, 5)}
        )
        self._pending_commands.append(cmd)
        return cmd

    def trigger_disaster(self, disaster_type: str = "Bão Thiên Tai") -> AdministratorCommand:
        """Tạo lệnh giáng họa thiên tai lên môi trường thế giới."""
        cmd = AdministratorCommand(
            command_type="TRIGGER_DISASTER",
            payload={"disaster": disaster_type}
        )
        self._pending_commands.append(cmd)
        return cmd
