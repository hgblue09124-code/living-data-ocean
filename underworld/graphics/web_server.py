...
Cài đặt tuân thủ tuyệt đối ranh giới Presentation Layer:
  ...
  - Hỗ trợ Background Thread tự động tick nhịp 2 giây khi bật Auto-Run.
...
class UnderworldWebHandler(BaseHTTPRequestHandler):
    ...
    def _start_auto_run_thread(cls):
        """Khởi chạy Background Thread tự động tick mỗi 2 giây."""
        ...
        def auto_ticker():
            while True:
                if cls.auto_run_active and cls.world_program and cls.admin:
                    ...
                    print(f" [NHỊP TỰ VẬN HÀNH 2S] Auto-Tick -> [Tick {tick}] Thời tiết: {weather} | Con người: {humans_cnt} cá thể")
                time.sleep(2.0)
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            st = self.world_program.get_state()
            self._log_console_activity(f"Quản trị viên {status_str} chế độ Tự vận hành 2s", st)
            payload = self.ui_program.generate_web_presentation(st)
            payload["auto_run"] = self.__class__.auto_run_active
            self._send_json(payload)
        elif path == "/api/step":
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
        ...
        button.active-mode { background-color: var(--btn-active-bg); }
...
<body>
    <div class="container">
        <header>
            ...
            <div class="subtitle">Thế Giới Tự Vận Hành Nhịp 2s | iPhone & Mobile Compatible</div>
        </header>
    ...
    <script>
                ...
                if (mod.type === 'world_state_summary') {
                    ...
                    mod.data.actions.forEach(act => {
                        ...
                        if (act.id === 'auto_2s' && isAutoRun) {
                            btn.innerText = '⏸️ Đang Auto 2s (Tắt)';
        ...
        // Tự động poll trạng thái mới nhất mỗi 2 giây
        setInterval(fetchPresentation, 2000);
        fetchPresentation();
    </script>
</body>
</html>"""
